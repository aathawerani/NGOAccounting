...
import {
  ...
  FileText, Printer, ChevronLeft, ChevronRight, BarChart2,
} from "lucide-react";
...
export default function TenantsPage() {
  ...
  // Keyboard shortcuts: Ctrl+N → new tenant, Escape → close modal
  useEffect(() => {
    function onKey(e) {
      if (e.key === "Escape" && modal) { closeModal(); return; }
      if ((e.ctrlKey || e.metaKey) && e.key === "n" && !modal) {
        e.preventDefault();
        openAdd();
      }
    }
  ...
  function openHistory(tenant) {
    setHistYear(new Date().getFullYear());
    setModal({ type: "history", tenant });
  }
  function closeModal() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Modal */}
      {modal && (
        <ModalOverlay onClose={closeModal} wide={modal.type === "statement" || modal.type === "history"}>
          {modal.type === "form" ? (
            <TenantFormModal
            ...
            <PaymentHistoryModal
              tenant={modal.tenant}
              year={histYear} setYear={setHistYear}
              onClose={closeModal}
              onOpenStatement={() => {
                setStmtYear(histYear);
                setModal({ type: "statement", tenant: modal.tenant });
              }} />
          ) : (
...
function TenantRow({ tenant, onEdit, onDelete, onStatement, onHistory }) {
    ...
    <tr className="hover:bg-gray-50/60 transition-colors group">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={onHistory} title="Payment History"
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
      ...
      <form onSubmit={onSubmit} className="p-6 space-y-4"
        onKeyDown={e => { if ((e.ctrlKey || e.metaKey) && e.key === "s") { e.preventDefault(); onSubmit(e); } }}>
        {error && (
...
function PaymentHistoryModal({ tenant, year, setYear, onClose, onOpenStatement }) {
  const [data, setData]       = useState(null);
  const [loading, setLoading] = useState(true);
  const currentYear = new Date().getFullYear();
  ...
  useEffect(() => {
    ...
    fetch(`${API}/tenants/${tenant.id}/payment-history?year=${year}`)
  ...
  const STATUS_STYLE = {
    ...
    SHORT:   "bg-amber-400 text-white",
    ADVANCE: "bg-red-400 text-white",
    NONE:    "bg-gray-100 text-gray-400",
  };
  const STATUS_LABEL = { PAID: "Paid", SHORT: "Partial", ADVANCE: "Advance", NONE: "No record" };
    ...
    <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 rounded-t-2xl">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-semibold text-gray-900 truncate">{tenant.name}</h2>
            <p className="text-xs text-gray-400">Payment history — {fmtSpace(tenant)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0 ml-4">
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg px-1 py-1">
      ...
      <div className="p-6 overflow-auto flex-1">
        {loading ? (
          ...
          <p className="text-center text-sm text-gray-400 py-12">Failed to load history.</p>
        ) : (
          <>
            {/* Legend */}
            <div className="flex items-center gap-4 mb-4 flex-wrap">
              {[["PAID","bg-emerald-500","Paid in full"],["SHORT","bg-amber-400","Partial payment"],
                ["ADVANCE","bg-red-400","Advance / no cash"],["NONE","bg-gray-200","No receipt"]].map(([k,bg,lbl]) => (
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              {data.months.map(m => (
                <div key={m.month_num}
                  className={cn("rounded-xl p-3 text-center transition-all", STATUS_STYLE[m.status])}>
                  <p className="text-xs font-bold uppercase tracking-wide opacity-90">{m.month_name}</p>
                  {m.receipts_count > 0 ? (
                    <>
                      <p className="text-[10px] mt-1 opacity-80">{STATUS_LABEL[m.status]}</p>
                      <p className="text-[11px] font-semibold mt-0.5">
                        {PKR(m.collected) ?? PKR(m.billed) ?? "—"}
                      </p>
                      {m.shortfall > 0 && (
                        <p className="text-[9px] opacity-75 mt-0.5">bal {PKR(m.shortfall)}</p>
                      )}
                    </>
                  ) : (
            ...
            <div className="grid grid-cols-3 gap-px bg-gray-100 rounded-xl overflow-hidden">
              {[
                { label: "Total Billed",      value: data.totals.billed,      color: "text-gray-800" },
                { label: "Total Collected",   value: data.totals.collected,   color: "text-emerald-600" },
                { label: "Outstanding",       value: data.totals.outstanding, color: data.totals.outstanding > 0 ? "text-red-500" : "text-emerald-600" },
              ].map(({ label, value, color }) => (
                <div key={label} className="bg-white px-4 py-3 text-center">
                  <p className="text-xs text-gray-400 uppercase tracking-wide">{label}</p>
                  <p className={cn("text-base font-bold mt-0.5 font-mono", color)}>
